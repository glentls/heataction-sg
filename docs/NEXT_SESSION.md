...
GitHub Actions CI (`.github/workflows/ci.yml`) now runs the regression suite plus a real-browser check of the main workflow (`tests/browser/main_workflow.mjs`) on every push/PR — verified locally, but **not yet confirmed green on GitHub's actual runners**, since nothing has been pushed there this session.
