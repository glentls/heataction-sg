...
- GitHub Actions CI (`.github/workflows/ci.yml`): the regression suite on Python 3.11 and 3.12 on every push to `main` and every pull request, plus a real-headless-Chrome check of the main coordinator workflow (`tests/browser/main_workflow.mjs`) that gates on the tests job passing first.
...
Confirm the new GitHub Actions workflow actually runs green on real GitHub infrastructure once pushed (it is verified locally but not yet on GitHub's runners; see below). Separately: run the coordinator usability protocol (docs/USABILITY_TEST.md) with a real, naive participant — this is the one piece of evidence in this project that genuinely cannot be produced by more engineering. Also: schedule the Databricks ingestion as a recurring Lakeflow Job, and once enough observed history exists, run chronological evaluation on observed WBGT and add an evaluated forecast artifact/inference path preserving the persistence baseline comparison. Historical API access and field validation remain separate pending tasks.
...
- Added `.github/workflows/ci.yml`: a `tests` job runs the full regression suite on Python 3.11 and 3.12 on every push to `main` and every pull request (no install step needed — `heataction` has zero runtime dependencies, and `python -m unittest discover` resolves it from the checked-out working directory, same as running it locally). A `browser-check` job, gated on `tests` passing, seeds the synthetic demo, starts the local server, launches headless Chrome (preinstalled on the `ubuntu-latest` runner image), and runs a new tracked browser check.
- Added `tests/browser/main_workflow.mjs`: a permanent, git-tracked version of the ad-hoc browser checks used throughout this project's recent sessions (`artifacts/check-*.mjs`, which are git-ignored and disposable). It exercises the same six actions as docs/USABILITY_TEST.md — budget change, recommendation explanation, lock persistence across a budget drop, scenario comparison, weather-history trend/age, CSV export — against demo mode, and fails on any JavaScript exception or wrong result. Like the usability protocol's scripted walkthrough, this is a mechanical regression check, not usability evidence; the file's own header comment says so.
- Verified the script standalone against a local demo server and headless Chrome (separate from CI) before wiring it into the workflow: all six checks passed. The workflow YAML itself was structurally validated by parsing it with PyYAML locally (temporarily installed, then removed — it is not a project dependency).
- Not yet done: the workflow has not actually run on GitHub's hosted runners, since nothing has been pushed there yet in this session. `google-chrome`'s exact availability/path and any first-run timing differences on the real `ubuntu-latest` image remain unconfirmed until it does.
