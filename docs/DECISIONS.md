...
## 012: Weather history chart uses a bounded window, not a fixed time range

The history chart requests a bounded *count* of recent points (`limit`, default 100), not a fixed wall-clock window (e.g. "last 24 hours"), because the synthetic demo runs on a fixed replay clock while observed collection has so far been sparse and irregular; a time-window filter would either be meaningless against the demo's fixed clock or return near-nothing against real data. The chart's own caption states the actual observed time span instead of assuming one. The one-hour persistence forecast point is included in the plotted time domain so it gets its own position on the axis, and is additionally distinguished by a dashed connector, a hollow marker and an explicit "(forecast)" label, since with sparse or long-spanning real data a one-hour gap alone would not be visually obvious.

## 013: Policy backtest ground truth reuses the shipped heat-category threshold

`heataction/policy_evaluation.py` scores each candidate allocation policy against "did the policy's chosen area(s) include one that is genuinely in the High WBGT category right now." That ground truth is computed with the same `heat_category` function and 33 °C threshold the shipped policy itself uses, not an independently sourced severity measure — this is a real limitation, stated in the tool's own output, not hidden. It was accepted rather than inventing a second, unvalidated severity definition, since a made-up independent threshold would be less defensible than reusing the one this project has already reviewed against the official advisory (DECISIONS.md #005). The backtest runs on the synthetic demo dataset only; per #002, that demonstrates the evaluation method and produces a real, reproducible number, but is not a real-world impact claim, and running it on today's very sparse observed history would not be statistically meaningful.
